...
export default class SqlServerFormatter {
  /**
  ...
  format(query) {
    if (!tokenizer) {
      tokenizer = new Tokenizer({
        ...
        stringTypes: [`""`, "N''", "''", "``", "[]"],
        openParens: ["(", "CASE", /WHEN( NOT)? MATCHED(.*?) THEN/g],
        closeParens: [")", "END", ";", /WHEN( NOT)? MATCHED(.*?) THEN/g],
        indexedPlaceholderTypes: ["?"],
        namedPlaceholderTypes: ["@", ":"],
        lineCommentTypes: ["--"]
      });
    }
    return new Formatter(this.cfg, tokenizer).format(query).replace(/\n;/g,";").replace(/;\s*/g,";\n");
  }
}
